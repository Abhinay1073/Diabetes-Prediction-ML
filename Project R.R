...
# Load the dataset
diabetes_data <- read.csv("D:/Downloads/diabetes.csv")  # Corrected file path
...
# Plot: Predicted probabilities by class for Logistic Regression
...
# Plot: Predicted probabilities by class for Naive Bayes
...
# Plot: SVM Decision Boundary using Glucose and BMI
...
# Add dummy values for the other columns in grid to avoid the 'object not found' error
for (col in setdiff(names(train_data), c("Glucose", "BMI", "Outcome"))) {
  grid[[col]] <- mean(train_data[[col]], na.rm = TRUE)
}

# Now predict with SVM model using the grid
grid$Predicted <- predict(svm_model, newdata = grid)

# Plot decision boundary for SVM
ggplot() +
  geom_tile(data = grid, aes(x = Glucose, y = BMI, fill = Predicted), alpha = 0.5) +
